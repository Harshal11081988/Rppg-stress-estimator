...
import numpy as np
import cv2
import cv2.data  # noqa: F401  -- must import explicitly; cv2.data is not always
                  # auto-bound as an attribute of `cv2` just from `import cv2`
                  # (this is what causes "AttributeError: module 'cv2' has no
                  # attribute 'data'" on some platforms, notably cloud hosts).
from scipy.signal import butter, filtfilt, detrend, find_peaks


def _resolve_face_cascade_path():
    """
    Locate the bundled Haar cascade XML robustly. Falls back to searching
    cv2's install directory directly in case cv2.data is unavailable for
    some reason even after the explicit import above.
    """
    ...
    raise RuntimeError(
        "Could not locate the bundled Haar cascade file "
        f"({filename}). Your OpenCV installation may be incomplete -- "
        "try reinstalling opencv-python-headless."
    )


FACE_CASCADE_PATH = _resolve_face_cascade_path()
